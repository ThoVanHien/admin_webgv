...
class suanguoigv extends Component{
	...
	async componentDidMount() {
		if (this.props.match.params.id !== 'new') {
			const ngv = await (await fetch(`/gvnhanh/nguoigv/${this.props.match.params.id}`)).json();
	...
	render(){
		...
		const {item} = this.state;
		const title = <h1 className="h3 mb-2 text-gray-800 pb-3">{item.hoten ? 'Cập nhật người giúp việc' : 'Thêm người giúp việc'}</h1>;
			...
			<div className="container-fluid">
				...
				<Form onSubmit={this.handleSubmit}>
					...
					<div className="row col">
						...
						<FormGroup className="col-md-3 mb-3">
							<Label for="stateOrProvince">Quê quán</Label>
							<Input type="text" name="quequan" id="quequan" value={item.quequan || ''}
							onChange={this.handleChange} autoComplete="address-level1" required/>
						</FormGroup>			

					<FormGroup className="col">
						<Button color="primary" type="submit">Save</Button>{' '}
						<Button color="secondary" tag={Link} to="/nguoigv">Cancel</Button>
					</FormGroup>
				</Form>	
			</div>
...
export default withRouter(suanguoigv);